...
export default function SettingsPage() {
  ...
  const handleRoleChange = async (adminId: string, newRole: string) => {
    try {
      ...
      console.log("Sending role update:", { adminId, payload });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (formData.username.trim() !== editingAdmin.username) {
      urlEncodedData.append("username", formData.username.trim());
    }
    if (formData.email.trim() !== editingAdmin.email) {
    ...
    if (!Array.from(urlEncodedData.entries()).length) {
    ...
    try {
      const res = await fetch(`/api/admins/${editingAdmin._id}/update`, {
        ...
        body: urlEncodedData.toString(), // ✅ Fixed: Send the correctly encoded data
      });
